...
SYSTEM_PROMPT = (
    ...
    "fit(적합) / unfit(부적합, 요건상 명백히 지원 불가) / unsure(아래 기준에서만 사용) 중 하나로 분류하고,\n"
    ...
    "분야·업종이 같은 범주인지 애매한 경우는 unsure 사유가 아니다 — 그런 경우에도 반드시 최선의 판단으로\n"
    "fit 또는 unfit 중 하나를 선택해라. unsure는 공고 자체에 제목 외의 실질적인 설명 내용이 없어서\n"
    "(예: 제목만 있고 사업 내용·대상·분야를 전혀 알 수 없는 경우) 판단에 필요한 정보가 정말로 없을 때만 써라."
)
